...
fetchAllDoctors(pageIndex: number = 0, pageSize: number = 5): void {
  try {
    const pageNumber = pageIndex + 1;
     // Call the service method with pagination parameters
  this.apiService.getAllDoctors(pageNumber, pageSize).subscribe(response => {
...
onPageChange(event: any): void {
  ...
  this.fetchAllDoctors(event.pageIndex, event.pageSize);
}
  ...
  deleteDoctor(deleteDoctor: string): void {
    console.log('Delete doctor:', deleteDoctor);
